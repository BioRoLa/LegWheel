...
def plot_corgi_robot(theta=np.deg2rad(90), beta=0.0, gamma=0.0):
    ...
    # 2. Plot Detailed Legs
    for i in range(4):
        kin = CorgiLegKinematics(i)
        # Use the new detailed 3D plot method from the model
...
if __name__ == "__main__":
    ...
    plot_corgi_robot(theta=np.deg2rad(110), beta=np.deg2rad(10), gamma=np.deg2rad(0))
