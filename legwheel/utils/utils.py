...
def create_command_csv_phi(theta_command, beta_command, file_name, transform=True):  # 4*n, 4*n
    ...
    motor_command = np.hstack((motor_command, -1 * np.ones((motor_command.shape[0], 4))))
            ...
            tramsform_theta.append(
                np.hstack(
                    (
                        ...
                        theta_command[i, 0] * np.ones(2000),
                    )
                )
            )
            tramsform_beta.append(
                np.hstack(
...
def parabolic_blends(p, t, tp=0.2, vi=0, vf=0):
    ...
    for i in range(n_points - 1):
        if i == 0:
            # constant speed
            parabolic_arr[2 * i + 1] = v[i + 1] * np.array([0, 1, -t[i]]) + np.array([0, 0, p[i]])
        else:
            # constant speed
            parabolic_arr[2 * i + 1] = v[i + 1] * np.array([0, 1, -t[i]]) + np.array([0, 0, p[i]])
        tmp = t[i + 1] - 0.5 * tp  # acceleration start time
        parabolic_arr[2 * i + 2] = parabolic_arr[2 * i + 1] + 0.5 * a[i + 1] * np.array(
            [1, -2 * tmp, tmp**2]
        )  # acceleration
