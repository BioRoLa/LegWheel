...
class ActiveBankingDynamics:
    ...
    def __init__(self, robot=None):
        ...
        # COM bias in Body Frame — represents chassis CoM offset only (SPRUNG_MASS).
        # NOTE: Leg unsprung mass (LEGWHEEL_MASS_PER_LEG ≈ 1.14 kg × 4) shifts CoM as joints move.
        # Current cornering stability assumes CoM ≈ body origin; add per-leg FK for higher accuracy.
        self.com_bias_x = RobotParams.COM_BIAS_X
        self.com_bias_y = RobotParams.COM_BIAS_Y
    ...
    def calculate_cornering_stability(
        self, theta_deg=75.0, beta_deg=0.0, gamma_bank_deg=0.0, turn_radius=2.0
    ):
        """
        ...
        """
        roll_deg, z_offset, wheel_pts, q_list = self.calculate_induced_roll(
            theta_deg, beta_deg, gamma_bank_deg
        )
        ...
        # 2. Determine the outer tipping boundary.
        # In FLU: Y is left(+), right(-). For a left turn, centrifugal force pushes to right(-).
        # Outer (tipping) edge = right side wheels (FR=1, RR=2); margin = CoM_Y − min(right_Y).
        if gamma_bank_deg >= 0:
            # Left turn: outer wheels are FR (index 1) and RR (index 2)
            y_outer = min(wheel_pts[1][1], wheel_pts[2][1])
            y_margin = y_com - y_outer
        else:
            # Right turn: outer wheels are FL (index 0) and RL (index 3)
            y_outer = max(wheel_pts[0][1], wheel_pts[3][1])
            y_margin = y_outer - y_com
