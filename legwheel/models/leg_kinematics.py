...
class LegKinematics:
    def __init__(self) -> None:
        ...
        self.ang_BCF = np.arccos(
            (self.l3**2 + self.l7**2 - self.l_BF**2) / (2 * self.l3 * self.l7)
        )  # angle BCF
    ...
    def forward(self, theta, beta, vector=True):
        ...
        self.n_elements = (
            0 if self.theta.ndim == 0 else self.theta.shape[0]
        )  # amount of theta given in an array, 0: single value.
        # Check theta range
    ...
    def calculate(self):
        ...
        self.ang_DBC = np.arccos(
            (self.l_BD**2 + self.l3**2 - self.l4**2) / (2 * self.l_BD * self.l3)
        )
        self.C_l = self.B_l + (self.D_l - self.B_l) * np.exp(-1j * (self.ang_DBC)) * (
            self.l3 / self.l_BD
        )  # OC = OB + BC
        self.F_l = self.C_l + (self.B_l - self.C_l) * np.exp(-1j * (self.ang_BCF)) * (
            self.l7 / self.l3
        )  # OF = OC + CF
        self.ang_OGF = np.arcsin(abs(self.F_l.imag) / self.l8)
        self.G = self.F_l.real - self.l8 * np.cos(self.ang_OGF)  # OG = OF - GF
        self.U_l = self.B_l + (self.C_l - self.B_l) * np.exp(1j * (self.ang_UBC)) * (
            self.R / self.l3
        )  # OOU = OB + BOU
        self.L_l = self.F_l + (self.G - self.F_l) * np.exp(1j * (self.ang_LFG)) * (
            self.R / self.l8
        )  # OOL = OF + FOL
        ...
        self.J_l = self.U_l + (self.R + self.foot_offset) * np.exp(
            1j * (np.deg2rad(135) + np.angle(self.H_l - self.U_l))
        )
        self.H_extend_l = self.U_l + (self.R + self.foot_offset) * np.exp(
            1j * (np.angle(self.H_l - self.U_l))
        )
        self.symmetry()
