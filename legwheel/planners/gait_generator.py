import legwheel.planners.trajectory_planning as TJ
import legwheel.models.leg_model as LegModel
import numpy as np
import matplotlib.pyplot as plt
import legwheel.visualization.plot_leg as PlotLeg
...
class leg_object:
    ...
    def extend(self, target_length=0, transform_included=False):
        ...
        self.local_Motion = (
            self.traj_curve * scale + self.traj_curve[: target_length % len(self.traj_curve)]
        )
...
class Gait_Generator:
    ...
    def __init__(
        ...
        # initialize
        self.Traj = TJ.TrajectoryPlanner(
            ...
            leg.leg.plot_by_angle(
                ...
                O=np.array(
                    [
                        self.offset_x[i]
                        + self.BL / 2
                        + (self.Traj.V * ((iteration - 5000) / 1000 if iteration > 5000 else 0)),
    ...
    def save(self, Dir="", output_file_name=""):
        if output_file_name == "":
            self.output_file_name = "".join(
                [
                    f"{self.gait_select}_Traj_",
...
if __name__ == "__main__":
    ...
    func_animation = FuncAnimation(
        fig, update, frames=frame_len, interval=ani_interval, repeat=False
    )
