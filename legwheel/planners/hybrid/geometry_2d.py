...
@dataclass(frozen=True)
class LinkSegment2D:
    """One existing LegWheel linkage segment expressed in hip-frame x-z."""
    ...
    def __post_init__(self) -> None:
        ...
        if points.shape != (2, 2):
            raise ValueError("points_hip_xz_m must have shape (2, 2).")
...
class SampledLegGeometry2D:
    ...
    hip_pose: HipPose2D
    link_segments_hip_xz_m: Sequence[LinkSegment2D] = field(default_factory=tuple)
    ...
    def __post_init__(self) -> None:
        ...
        link_segments = tuple(self.link_segments_hip_xz_m)
        if not all(isinstance(item, LinkSegment2D) for item in link_segments):
            raise TypeError("link_segments_hip_xz_m must contain LinkSegment2D values.")
        angles = angles.copy()
        ...
        object.__setattr__(self, "link_segments_hip_xz_m", link_segments)
    ...
    @property
    def link_segments_world_xz_m(self) -> tuple[NDArray[np.float64], ...]:
        """Return the optional existing linkage segments in world x-z."""

        return tuple(
            transform_points_hip_to_world(segment.points_hip_xz_m, self.hip_pose)
            for segment in self.link_segments_hip_xz_m
        )
...
def sampled_leg_geometry_from_legacy_records(
    ...
    hip_pose: HipPose2D,
    *,
    link_segments_hip_xz_m: Sequence[LinkSegment2D] = (),
    ...
    return SampledLegGeometry2D(
        ...
        alpha_rad,
        hip_pose,
        link_segments_hip_xz_m=link_segments_hip_xz_m,
    )


def legacy_leg_link_segments_2d(theta: float, beta: float) -> tuple[LinkSegment2D, ...]:
    """Expose the same six linkage bars already rendered by ``PlotLeg``.

    This is numeric collision geometry only; it does not introduce a second
    kinematic model.  The source points are generated by the existing
    ``PlotLeg.forward(theta, beta)`` implementation.
    """
    ...
    def point(name: str) -> NDArray[np.float64]:
        if name == "O":
            return origin
        value = np.asarray(getattr(leg, name), dtype=float)
        if value.shape != (2,):
            raise ValueError(f"legacy link point {name!r} must have shape (2,).")
        return value
    ...
    definitions = (
        ...
        ("AE_link_left", "A_l", "E"),
        ("AE_link_right", "A_r", "E"),
        ("CD_link_left", "C_l", "D_l"),
        ("CD_link_right", "C_r", "D_r"),
    )
    return tuple(
        LinkSegment2D(
            segment_id=segment_id,
            points_hip_xz_m=np.array([point(start), point(end)], dtype=float),
        )
        for segment_id, start, end in definitions
    )
