...
class TrajectoryPlanner:
    ...
    def __init__(
        ...
        self.swing = swing.SwingLegPlanner(
            dt=dt,
            T_sw=self.T * self.duty * (1 - self.overlap),
            T_st=self.T * (1 - self.duty * (1 - self.overlap)),
        )  # Initialize swing leg planner
    ...
    def get_init_tb(self):
        ...
        """
        func = (
            lambda x: (self.H) * np.tan(x) + self.leg["foot_radius"] * x - 3 * self.step_length / 8
        )
        ...
        solver = Solver(
            ...
            max_iter=100,
            function=func,
            derivative=lambda x: (self.H) * np.sec(x) ** 2
            + self.leg["foot_radius"]
            - 3 * self.step_length / 8,
        )
        ...
        self.L = (
            2 * OO_r_Dist * np.sin(self.beta0)
        )  # distance of ground contact to hip joint in X axis
        self.D = (
            self.L + self.leg.foot_radius * 2 * self.beta0
        ) / 3  # movement of hip joint in X axis during one step
    ...
    def generate_swing_trajectory(self):
        ...
        self.swing_trajectory = self.swing.solveSwingTrajectory(
            self.G_lo, self.G_td, self.step_height, self.v_lo, self.v_td
        )
    ...
    def move(self):
        ...
        for t in np.arange(0, self.T * (1 - self.duty * (1 - self.overlap)), self.dt):
            if t < self.T / 4 * 3:
                ...
                solver = Solver(
                    ...
                    tol=1e-9,
                    max_iter=100,
                    function=lambda beta: self.H * (np.sin(self.beta0) - np.sin(beta))
                    + self.leg["foot_radius"] * (self.beta0 - beta)
                    - self.V * t,
        ...
        # swing phase
        self.swing_points = [
            self.swing_trajectory.getFootendPoint(_)
            for _ in np.linspace(0, 1, int(self.T * self.duty * (1 - self.overlap) / self.dt))
        ]  # get points in swing trajectory [x,y]
        # transfer to polar coordinates
        self.swing_points_polar = list(
            map(self.cartesian_to_polar, self.swing_points)
        )  # transform to polar coordinates then transfer to command space
        ang_offset = np.pi / 2  # angle offset for swing trajectory

        # transform to command space
        self.swing_points_cmd = list(
            map(lambda x: [inv_G_dist_poly(x[0]), x[1] + ang_offset], self.swing_points_polar)
        )  # transform to command space

        self.swing_points_cmd = list(
            map(
                lambda x: [x[0] if x[0] <= np.deg2rad(160) else np.deg2rad(160), x[1]],
                self.swing_points_cmd,
            )
        )  # transform to command space
        self.swing_points_cmd = list(
            map(
                lambda x: [x[0] if x[0] >= np.deg2rad(17) else np.deg2rad(17), x[1]],
                self.swing_points_cmd,
            )
        )  # transform to command space
...
if __name__ == "__main__":
    ...
    def plot_point(ax, point, color="red", plot_leg=PlotLeg()):
        point = ax.plot(
            point[0],
            point[1],
            marker="o",
            color=color,
            markersize=plot_leg.leg_shape.mark_size,
            zorder=plot_leg.leg_shape.zorder + 0.00001,
        )[0]
        return point
    ...
    if animate:
        ...
        def update(frame):
            ...
            hip_movement = traj_planner.H * (np.sin(beta0) - np.sin(beta)) + leg.foot_radius * (
                beta0 - beta
            )  # calculate hip_movement
            # hip_movement = traj_planner.V*t
            # hip_movement = 0
            ground_contact_point = plot_leg.rim_point(np.rad2deg(-beta)) + np.array(
                [hip_movement, 0]
            )
            ...
            plot_leg.plot_by_angle(
                theta, beta, O=np.array([hip_movement, 0]), ax=ax
            )  # plot the leg
            plot_point(
                ax, ground_contact_point, color="red", plot_leg=plot_leg
            )  # plot the ground contact point
            plot_point(
                ax, np.array([[0, hip_movement], [0, 0]]), color="blue", plot_leg=plot_leg
            )  # plot the hip joint trajectory
        ...
        if save_data:
            ...
            output_file_name = "".join(
                [
                    "Output_datas/Single_leg_trajectory",
            ...
            pd.DataFrame(traj_planner.cmd, columns=["theta", "beta"]).to_csv(
