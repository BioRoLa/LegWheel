...
class PlotLeg(LegModel):
    ...
    class LegShape:
        def __init__(self, leg_model, O):
            ...
            # Color palette
            self.color_set = [plt.get_cmap("Set1")(i) for i in range(plt.get_cmap("Set1").N)] + [
                plt.get_cmap("Set2")(i) for i in range(plt.get_cmap("Set2").N)
            ]
            self.color_label = {
        ...
        def _update_geometry(self, tyre_offset=None):
            ...
            # Arcs
            c = 0.0012  # Clearance
            self.upper_rim_r = self.RimObj(
                *self._make_arc(lm.F_r, lm.H_r, lm.U_r, lm.r / 2 - c, "Upper_Rim")
            )
            self.upper_rim_l = self.RimObj(
                *self._make_arc(lm.H_l, lm.F_l, lm.U_l, lm.r / 2 - c, "Upper_Rim")
            )
            self.lower_rim_r = self.RimObj(
                *self._make_arc(lm.G, lm.F_r, lm.L_r, lm.r / 2 - c, "Lower_Rim")
            )
            self.lower_rim_l = self.RimObj(
                *self._make_arc(lm.F_l, lm.G, lm.L_l, lm.r / 2 - c, "Lower_Rim")
            )
            self.foot_rim = self.RimObj(*self._make_arc(lm.I_l, lm.I_r, lm.O_r, toff, "Foot_Rim"))
            self.upper_rim_r_f = self.RimObj(
                *self._make_arc(
                    lm.J_r, lm.H_extend_r, lm.U_r, max(toff - c, 0), "Upper_Tyre", z_off=0.0002
                )
            )
            self.upper_rim_l_f = self.RimObj(
                *self._make_arc(
                    lm.H_extend_l, lm.J_l, lm.U_l, max(toff - c, 0), "Upper_Tyre", z_off=0.0002
                )
            )
            ...
            self.H_extend_joint_l = self._make_circle(
                lm.H_extend_l, max(toff, 0), "Upper_Tyre", z_off=0.0002
            )
            self.H_extend_joint_r = self._make_circle(
                lm.H_extend_r, max(toff, 0), "Upper_Tyre", z_off=0.0002
            )
            ...
            if self.Construction:
                self.U_L_Construction_bar_1 = self._make_dashed_line(
                    lm.U_l, lm.H_extend_l, "Construction_Line"
                )
                self.U_L_Construction_bar_2 = self._make_dashed_line(
                    lm.U_l, lm.J_l, "Construction_Line"
                )
                self.U_R_Construction_bar_1 = self._make_dashed_line(
                    lm.U_r, lm.H_extend_r, "Construction_Line"
                )
                self.U_R_Construction_bar_2 = self._make_dashed_line(
                    lm.U_r, lm.J_r, "Construction_Line"
                )
                self.foot_Construction_bar_1 = self._make_dashed_line(
                    lm.I_l, lm.O_r, "Construction_Line"
                )
                self.foot_Construction_bar_2 = self._make_dashed_line(
                    lm.I_r, lm.O_r, "Construction_Line"
                )
                g = self._to_xy(lm.G)
                self.center_Construction_axis_bar = self._make_dashed_line(
                    g * 1.3, -g * 1.1, "Axis", linestyle="dashdot"
                )
        ...
        def _make_arc(self, p1, p2, o, offset, color_key, z_off=0.0):
            ...
            fill = Wedge(
                ...
                width=2 * offset,
                facecolor=(1, 1, 1, self.link_alpha),
                zorder=self.zorder + z_off - 0.0001,
            )
        ...
        def _make_circle(self, o, r, color_key, z_off=0.0):
            ...
            return Circle(
                ...
                radius=r,
                facecolor=(1, 1, 1, self.link_alpha),
                edgecolor=self.color_label[color_key],
                linewidth=self.line_width,
                zorder=self.zorder + z_off,
            )
    ...
    def plot_leg(self, theta, beta, O, ax):
        ...
        for key, val in self.leg_shape.__dict__.items():
            if "rim" in key and hasattr(val, "arc"):
                ax.add_patch(val.arc[0])
                ax.add_patch(val.arc[1])
                if self.leg_shape.link_alpha > 0:
                    ax.add_patch(val.arc_fill)
            elif "joint" in key:
                ax.add_patch(val)
            elif "bar" in key:
