...
if __name__ == "__main__":
    ...
    for ri, roll in enumerate(rolls):
        for gi, gl in enumerate(gammas):
            ...
            for theta in thetas:
                ...
                if m > best_m:
                    best_m = m; best_t = theta
            smap[gi, ri] = best_m
            theta_map[gi, ri] = best_t
        best_gi = np.argmax(smap[:, ri])
        print(f"  Roll={roll:3.0f}°  best_margin={smap[:, ri].max():+.3f}  "
    ...
    fig.suptitle("Quasi-Static Self-Righting  (θ=75°, β=0°)\n"
                 "Stability margin over Roll × γ_lower  (γ_upper=0°)",
                 fontsize=12)

    # Panel 1: stability heatmap
    ax = axes[0]
    cmap = plt.cm.RdYlGn
    im = ax.pcolormesh(rolls, gammas, smap,
                       cmap=cmap, vmin=-1.0, vmax=1.0, shading="auto")
    ax.contour(rolls, gammas, smap, levels=[0],
               colors="black", linewidths=1.5, linestyles="--")
    # Optimal path line
    ax.plot(rolls, best_gamma, "k-o", ms=5, lw=2, label="Optimal γ path")
    ax.set_xlabel("Roll (deg)"); ax.set_ylabel("γ_lower (deg)")
    ...
    ax.fill_between(rolls, best_margin, 0,
    ...
    ax.set_xlabel("Roll (deg)"); ax.set_ylabel("Norm. CoM margin")
    ...
    for roll_m, lbl in [(180, "S4"), (0, "S0")]:
        for a in axes:
            ...
            a.text(roll_m, a.get_ylim()[1] if hasattr(a, 'get_ylim') else 1,
                   lbl, fontsize=7, color="steelblue", ha="center", va="top")
    ...
    print("\n=== Quasi-Static Waypoints (Roll 180→0) ===")
