...
import os
EXP = os.environ.get("HW_EXP", "/home/chang/corgi_ws/icra hybrid/corgi-research/05_Experiments/0913exp")
TAG = os.environ.get("HW_TAG", "top930base")
ORIN = os.environ.get("HW_ORIN", "0913_900mm_v1_{trial}.csv")
VICON = os.environ.get("HW_VICON", "0913_900mm_v1_{trial}.csv")
NOTES = os.environ.get("HW_NOTES", "/home/chang/corgi_ws/icra hybrid/LegWheel/hybrid_note/notes/day14")
...
hw = np.array([[float(v) for v in r] for r in csv.reader(open(f"{NOTES}/day14_step3_100mm_roll_{TAG}_hardware.csv"))])
ev = [r for r in csv.DictReader(open(f"{NOTES}/day14_step3_100mm_roll_{TAG}_events.csv")) if r["row_kind"] == "swing"]
...
_ax = [r for r in csv.DictReader(open(f"{NOTES}/day14_step3_100mm_roll_{TAG}_events.csv")) if r["row_kind"] == "axle_profile"]
front_kn = [(float(r["hip_x_mm"]), float(r["hip_z_mm"]) - 219.4) for r in _ax if r["axle"] == "front"]
rear_kn = [(float(r["hip_x_mm"]), float(r["hip_z_mm"]) - 219.4) for r in _ax if r["axle"] == "rear"]
def front_z(hx): return np.interp(hx, [k[0] for k in front_kn], [k[1] for k in front_kn])
def rear_z(tp):
    return float(np.interp(plan_x(tp) - 255, [k[0] for k in rear_kn], [k[1] for k in rear_kn]))

# ---- Orin
rows = list(csv.DictReader(open(f"{EXP}/orin/" + ORIN.format(trial=trial))))
...
# ---- Vicon
lines = open(f"{EXP}/vicon/" + VICON.format(trial=trial), encoding="utf-8-sig").read().splitlines()
...
_win = [(f"{r['leg']} {r['kind'][:8]}", {"LH": "O41", "RH": "O31"}[r["leg"]], {"LH": "d", "RH": "c"}[r["leg"]], float(r["start_s"]) - 0.05, float(r["end_s"]) + 0.05)
        for r in ev if r["leg"] in ("LH", "RH") and 3.0 < float(r["start_s"]) < 9.0]
for leg, mk, col, t0, t1 in _win:
...
fine = list(np.arange(3.0, 4.4, 0.1)) + list(np.arange(6.8, 9.0, 0.1)) + list(np.arange(15.5, 18.6, 0.1))
